...
| **Dangerous** | Impersonation, bulk-cleanup, backups are service-role privileged; most edge fns run `verify_jwt=false` and authenticate in-function via `_shared/auth.ts`, provider signatures, or token scoping — ~20 of 108 keep gateway JWT verification (see ARCHITECTURE_BOUNDARIES; forged-JWT service-role bypass was the audit P0 — now fixed). |
