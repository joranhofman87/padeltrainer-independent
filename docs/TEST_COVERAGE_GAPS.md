...
**Inventory today (2026-07-18):** 427 vitest files (`src/**/*.test.{ts,tsx}`), of which **99 are PGlite integration** (`*.pglite.test.ts`) running the real lib/migration SQL against real Postgres; **29 deno `_shared` test files**; **26 playwright e2e specs** (15 `e2e/` + 9 `e2e/local/` mock-Mollie money-path + 2 `tests/`); **44 `scripts/db/rehearse-*`** SQL-invariant scripts (`db:rehearse:all` gates a subset). 98 of the vitest files are `.tsx` component tests.
...
## Well covered (person unification — phases 1–3.6, all PGlite against the real migration SQL)
...
- **Phase 3.3 (detail refs, booking gate, attendance):** [`getPersonRefsForScope.pglite.test.ts`](../src/test/getPersonRefsForScope.pglite.test.ts) (+IDOR guards), [`canBookMemberWindowPerson.pglite.test.ts`](../src/test/canBookMemberWindowPerson.pglite.test.ts), [`attendancePersonRls.pglite.test.ts`](../src/test/attendancePersonRls.pglite.test.ts) (real RLS under `SET ROLE authenticated`), [`sessionReportsPlayerPrivacy.pglite.test.ts`](../src/test/sessionReportsPlayerPrivacy.pglite.test.ts), [`pendingAttendance.test.ts`](../src/lib/pendingAttendance.test.ts) (guest-seated reportability).
...
- **Phase 3.6 (dashboards):** [`dashboardPersonCounts.pglite.test.ts`](../src/test/dashboardPersonCounts.pglite.test.ts) — person-correct new-player head-counts on BOTH dashboard fns (merged-person-counts-once, frozen-guest-own-person, dual-key-accountless = guest, login-derived `is_registered` on every arm, unstamped degradations, anon lockdown; 10 pins), plus the detail-page rating-fetch personRefs pins in `AcademyPlayerDetail.test.tsx` / `TrainerPlayerDetail.test.tsx`.
