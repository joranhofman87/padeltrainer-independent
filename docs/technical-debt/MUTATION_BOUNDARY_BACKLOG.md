...
| P1-b | `src/pages/academy/AcademyCyclusOverview.tsx:864,871` (2 allowlisted) | Targeted bulk price: `availability_slots.update({price_per_session})` (chunked 500) + `cycles.update({price_per_session})` | Writes the price source-of-truth (ADR [`0002`](../adr/0002-slot-is-price-source-of-truth.md)) then relies on a *separate* `syncInvoicesAfterPriceChange` follow-up; if a future edit drops the sync, invoices silently drift | **Deliberate today** (documented, groups by `cyclus_id+trainer_id` so it can't use whole-cyclus `updateCyclePricing`). Recommended: a `setTargetedCyclePrice(slotIds, cycleIds, price)` facade that bundles both writes **plus** the invoice resync atomically, so the sync can't be forgotten | Medium; characterize the 3-step (slots → cycle → resync) then extract |
