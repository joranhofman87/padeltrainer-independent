...
| **Edge functions** | `supabase/functions/**` | The server/backend boundary: caller auth, money (server-trusted pricing), cross-tenant work, anything needing the **service role** or a third-party secret (Mollie, Resend). ~96 fns, `verify_jwt=false`, each self-authenticates via `_shared/auth.ts`. | Trust client-supplied prices or a decodable JWT claim for privilege. Skip `requireUser` / `requireServiceRole`. |
...
- **Never** issue a raw high-risk mutation. A new `supabase.from('bookings' | 'availability_slots' | 'cycles' | 'registrations' | 'invoices' | 'slot_priority_claims' | 'email_campaign_recipients').insert/update/delete` in `src/pages` or `src/components` **fails `src/test/mutationBoundary.test.ts`** (a shrink-only allowlist) and points you to the owner. Reads and low-risk writes are fine.
