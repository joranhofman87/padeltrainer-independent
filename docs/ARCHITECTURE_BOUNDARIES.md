...
| **Edge functions** | `supabase/functions/**` | The server/backend boundary: caller auth, money (server-trusted pricing), cross-tenant work, anything needing the **service role** or a third-party secret (Mollie, Resend). 108 fns (2026-08-08 count), most `verify_jwt=false` (89 config entries; 2 gateway-JWT) with self-authentication via `_shared/auth.ts`. | Trust client-supplied prices or a decodable JWT claim for privilege. Skip `requireUser` / `requireServiceRole`. |
...
- **Never** issue a raw high-risk mutation. A `supabase.from('bookings' | 'availability_slots' | 'cycles' | 'registrations' | 'invoices' | 'slot_priority_claims' | 'email_campaign_recipients').insert/update/delete` in a NEW file under `src/pages`/`src/components`, or one raising an existing file's count, **fails `src/test/mutationBoundary.test.ts`** (per-file count ceiling; count-neutral swaps pass mechanically — the rule, not the test, is the contract) and points you to the owner. Reads and low-risk writes are fine.
...
| A new dangerous mutation of any kind | **extend the existing facade / RPC** in the MUTATION_BOUNDARIES "Allowed boundary" column | Never add a raw `supabase.from(...)` write in a page — `mutationBoundary.test.ts` fails. |
