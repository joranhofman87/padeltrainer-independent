...
- **e2e:** `e2e/rls-health.spec.ts`, `e2e/roles.spec.ts`. Every new edge fn MUST self-authenticate via `_shared/auth.ts` (most fns run verify_jwt=false by design — 88 configured of 108, ~20 effectively gateway-verified; `check:edge-config` enforces a curated allowlist, so add new public/self-authenticating fns to it deliberately).
