...
|---|---|---|---|---|---|---|---|
| 1 | **Create booking** | lib facade (`insertBookings` / `insertBookingSingle`) or RPC `book_slot_for_payment`; proposal finalization inserts via service-role RPC `finalize_cycle_proposals` (no capacity lock yet — backlog B-1) | `src/lib/bookings.ts:236,258`; guests via `src/lib/slotBookingWrite.ts:141` (`insertGuestsIntoSlots`); pay-first path RPC `book_slot_for_payment` | No — all page/component inserts routed to lib | Partial-unique `uniq_active_booking_per_slot_{player,guest}` (no double-book) + `enforce_booking_slot_tier` capacity trigger + RLS | `src/lib/bookings.test.ts`, `slotBookingWrite`/`bookForPlayerBooking.test.ts`; PGlite rehearsals | Low–med. Trigger-guarded for authenticated inserts; the old service-role single-slot double-insert was the P0, fixed #183. Remaining (corrected 2026-08-08): service-role `finalize_cycle_proposals` inserts bookings with no capacity lock/recount (backlog B-1). |
...
`src/test/mutationBoundary.test.ts` is a **per-file count-ceiling allowlist** (shrink-only by convention, same model as the eslint suppression baseline). A direct high-risk write on `bookings / availability_slots / cycles / registrations / invoices / slot_priority_claims / email_campaign_recipients` in a NEW file under `src/pages`/`src/components`, or one that raises an existing file's count above its allowlisted number, **fails the test** and points the author to a `src/lib/*` owner. A count-neutral swap, or headroom under a stale ceiling, passes mechanically — the rule, not the test, is the contract. `src/lib/**` is the domain layer and is intentionally not scanned. Regenerate the baseline only when a write *moves behind* an owner (it may only shrink).
