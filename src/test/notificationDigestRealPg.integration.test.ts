...
beforeAll(async () => {
  ...
  await c.query(`
    ...
    -- replicate Supabase's default privileges so this server exercises the exact prod ACL semantics
...
describe('10c-a1 real-Postgres — post-lockdown ACL under an actual service_role session', () => {
  it('a ledger INSERT succeeds under SET ROLE service_role with ONLY sequence USAGE (proves USAGE is sufficient)', async () => {
    const c = new Client({ connectionString: url }); await c.connect();
    try {
      const g = await newGroup(c); // as owner
      await c.query('SET ROLE service_role');
      // the ledger seq is GENERATED BY DEFAULT AS IDENTITY → the INSERT calls nextval(); only USAGE is granted.
      const r = await c.query(`INSERT INTO public.notification_digest_group_attempts (digest_group_id, action)
        VALUES ($1,'materialized') RETURNING seq`, [g]);
      await c.query('RESET ROLE');
      expect(Number(r.rows[0].seq)).toBeGreaterThan(0); // nextval worked with USAGE alone
    } finally { await c.query('RESET ROLE').catch(() => {}); await c.end(); }
  });

  it('service_role cannot DELETE or TRUNCATE an audit table on the real server (lockdown holds under SET ROLE)', async () => {
    const c = new Client({ connectionString: url }); await c.connect();
    try {
      const g = await newGroup(c);
      await c.query(`INSERT INTO public.notification_digest_group_attempts (digest_group_id, action) VALUES ($1,'materialized')`, [g]);
      await c.query('SET ROLE service_role');
      await expect(c.query(`DELETE FROM public.notification_digest_group_attempts WHERE digest_group_id=$1`, [g])).rejects.toThrow(/permission denied/i);
      await expect(c.query(`TRUNCATE public.notification_digest_group_attempts`)).rejects.toThrow(/permission denied/i);
      await c.query('RESET ROLE');
    } finally { await c.query('RESET ROLE').catch(() => {}); await c.end(); }
  });
});
