...
describe('the reproduced merge_guest_players differs from the shipped one only by the U2 insert', () => {
  ...
  it('the insert moves the CANONICAL id, and touches no legacy source', () => {
    const reproduced = extractMerge(REPRODUCED);
    const inserted = reproduced.match(INSERTED)?.[0] ?? '';

    // ONLY when the source person actually dies, and only onto a real successor. Repointing
    // unconditionally would hand a surviving person's commands to somebody else.
    ...
    // ...and there is no second column to keep in step. A receipt that stored a guest reference is
    // what made this a rule about consistency instead of a single assignment.
    expect(inserted).not.toMatch(/guest_player_id/);
  });
