...
const PRESETS: Record<string, ScoringWeights> = {
  balanced: { time_match: 35, preferred_trainer: 20, level_compatible: 15, priority_bonus: 10, capacity_available: 10, sessions_per_week: 10 },
  timeFocused: { time_match: 50, preferred_trainer: 10, level_compatible: 15, priority_bonus: 10, capacity_available: 10, sessions_per_week: 5 },
  levelFocused: { time_match: 25, preferred_trainer: 15, level_compatible: 35, priority_bonus: 5, capacity_available: 10, sessions_per_week: 10 },
};

const SUGGESTED_SPREADS: Record<string, number> = {
  knltb: 0.5, playtomic: 0.5, fep: 0.5, tennis_vlaanderen: 75, lta: 0.25,
};
...
function WeightSlider({ label, helpText, value, onChange, icon }: WeightSliderProps) {
  return (
    <div className="space-y-3">
      ...
      <Slider value={[value]} onValueChange={(v) => onChange(v[0])} min={0} max={100} step={5} className="w-full" />
    </div>
  );
}
...
interface ScoringWeightsPanelProps {
  ...
  showRatingSpread?: boolean;
}

export function ScoringWeightsPanel({ weights, onWeightsChange, showRatingSpread = true }: ScoringWeightsPanelProps) {
  ...
  return (
    <div className="space-y-6 py-2">
      ...
      <div className="flex gap-2">
        <Button variant={activePreset === 'balanced' ? 'default' : 'outline'} size="sm" onClick={() => applyPreset('balanced')} className="flex-1">
          <Zap className="h-3.5 w-3.5 mr-1.5" />{t('proposals.weights.presets.balanced')}
        </Button>
        <Button variant={activePreset === 'timeFocused' ? 'default' : 'outline'} size="sm" onClick={() => applyPreset('timeFocused')} className="flex-1">
          <Clock className="h-3.5 w-3.5 mr-1.5" />{t('proposals.weights.presets.timeFocused')}
        </Button>
        <Button variant={activePreset === 'levelFocused' ? 'default' : 'outline'} size="sm" onClick={() => applyPreset('levelFocused')} className="flex-1">
          <TrendingUp className="h-3.5 w-3.5 mr-1.5" />{t('proposals.weights.presets.levelFocused')}
        </Button>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select value={ratingSpread.ratingSystem} onValueChange={(value) => setRatingSpread(prev => ({ ...prev, ratingSystem: value, maxSpread: SUGGESTED_SPREADS[value] || null }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {ratingSystems.map(sys => (<SelectItem key={sys.code} value={sys.code}>{sys.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setRatingSpread(prev => ({ ...prev, maxSpread: e.target.value ? parseFloat(e.target.value) : null }))}
