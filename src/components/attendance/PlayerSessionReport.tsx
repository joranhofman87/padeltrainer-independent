...
export function PlayerSessionReport({
  ...
  const sessionHappened = report?.sessionHappened ?? null;
  ...
  const choose = (value: boolean) => {
    ...
    reportAttendance.mutate(
      ...
      { onError: () => toast({ title: t('attendance.saveError', 'Failed to save'), variant: 'destructive' }) },
    );
  };
    ...
    <div className={cn('space-y-3', className)}>
      <div className="flex items-center justify-between">
        ...
        {sessionHappened !== null && (
          <Badge variant="outline" className="h-5 gap-0.5 border-emerald-300 px-1.5 text-[10px] text-emerald-600">
