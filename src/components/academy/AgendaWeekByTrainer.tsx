        ...
        <div className="min-w-[820px]">
          ...
          {visibleTrainers.map((tr, i) => {
            ...
            return (
              <div
                key={tr.id}
                className="grid grid-cols-[180px_repeat(7,minmax(0,1fr))_72px] items-center gap-1 border-b last:border-b-0 px-1 py-2"
              >
                <button
                  type="button"
                  onClick={() => onTrainerClick?.(tr.id)}
                  className="flex items-center gap-2 px-2 py-1 text-left rounded-md hover:bg-accent/40 transition-colors min-w-0"
                >
