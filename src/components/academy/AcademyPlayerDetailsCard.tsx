...
export function AcademyPlayerDetailsCard({
  ...
  async function handleSave() {
    ...
    try {
      ...
      logger.error(
        'Error saving academy player details',
        err instanceof Error ? err : new Error(String(err)),
        { component: 'AcademyPlayerDetailsCard' },
      );
      toast({
        title: tCommon('error', 'Error'),
  ...
  return (
    <Card className={flushOnMobileCardClass()} data-testid="academy-player-details-card">
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0">
        ...
        {!editing ? (
          <Button
            variant="outline"
            size="sm"
            data-testid="academy-player-details-edit"
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" data-testid="academy-player-details-save" onClick={() => void handleSave()} disabled={saving}>
        ...
        {!editing ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="player-details-name">{t('players.name', 'Name')} *</Label>
              <Input
                id="player-details-name"
                data-testid="academy-player-details-name"
                value={form.name}
                onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="player-details-email">{t('players.email', 'Email')}</Label>
              <Input
                id="player-details-email"
                data-testid="academy-player-details-email"
                type="email"
              ...
              />
              {emailReadOnly && (
                <p className="text-xs text-muted-foreground" data-testid="academy-player-email-readonly-help">
                  {t(
            ...
            <div className="space-y-2">
              <Label htmlFor="player-details-rating">{t('players.detail.knltbLevel', 'KNLTB level')}</Label>
              <Input
                id="player-details-rating"
                data-testid="academy-player-details-rating"
                type="number"
                min={1}
                max={10}
                step={0.1}
            ...
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="player-details-notes">{t('players.notes', 'Notes')}</Label>
              <Textarea
                id="player-details-notes"
                data-testid="academy-player-details-notes"
                rows={4}
                value={form.notes}
