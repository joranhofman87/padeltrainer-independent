...
- [ ] **P0 payments observability (migration + 2-3 edge fns, deploy together):** 1) apply `20260712100000_reconcile_payments_service_role.sql` (gate-only change: the nightly health check may run the read-only reconciler; user JWTs still admin-only). 2) `supabase functions deploy invoice-health-check mollie-webhook slack-notify --project-ref ficwbdrzefmblkbkomzw` — invoice-health-check gains the LOST-WEBHOOK detector (asks Mollie about recent unpaid-locally payments, alerts `paid_at_mollie_not_locally` = money captured with nothing to show — the gap where a guest's payment could vanish silently) + folds reconcile_payments findings into the nightly Slack alert; mollie-webhook bundles the extracted `_shared/mollie-token-resolution.ts` (behavior identical); slack-notify gets the 💓 heartbeat title (optional — unknown events render fine). The daily 💓 heartbeat itself ships via Vercel automatically: NO heartbeat some morning = the cron/alerting pipe is down.
