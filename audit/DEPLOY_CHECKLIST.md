...
- [ ] **`20260707100000_book_guest_cart_for_payment.sql`** (cart booking PR 1) — new `book_guest_cart_for_payment` RPC (guest multi-session cart holds; clone of the cyclus RPC with single-slot per-item guards + id-carrying errors). Additive, no consumer until `create-guest-cart-payment` deploys (cart PR 2) — safe to apply any time, **must be live before PR 2's function**. Do NOT regenerate `types.ts` (service-role-only RPC; regen would drag the stale-since-#273 diff).
