...
- [ ] **trainer double-booking guard (migration only, order-free):** apply `20260708100000_trainer_slot_overlap_guard.sql` — AFTER-row trigger on `availability_slots` refusing any INSERT/time-UPDATE whose [start,end) range overlaps another slot of the SAME trainer (`trainer_slot_overlap`, race-proof via per-trainer advisory lock). The DB backstop behind the client-side dedup fixes (quick-generator now overlap-based; bulk-create's dedup was format-broken and silently dead). No new columns are selected, so the frontend works before OR after the migration — apply whenever convenient. Existing prod overlaps keep working (the trigger gates new writes only); to SEE current duplicates first, run:
