...
| 3c | *(owner)* wire the EXTERNAL monitor | Deploy `notif-liveness` (the endpoint that makes the service_role-only `public.notif_digest_worker_liveness()` readable by an uptime provider without handing it the service-role key), set `NOTIF_LIVENESS_TOKEN`, point an INDEPENDENT uptime service at it, and prove it alerts. Before the canary, so it is watching from the first send. Full contract, states and credentials: [`docs/NOTIFICATION_OPERATIONS.md`](../../../docs/NOTIFICATION_OPERATIONS.md). |
...
| 7a | *(owner)* `NOTIF_LIVENESS_EXPECT_ARMED=true` **BEFORE** step 7 | The monitor cannot infer activation: between the canary and arming, the liveness row is identical to a disarmed live cron. Declare it first (the endpoint then reports `cron_disarmed`, which is TRUE), confirm the endpoint reports exactly that **state** — 503 alone is ambiguous — then arm. Arming first shows `unexpectedly_armed`: a live cron the monitor is not yet watching for staleness. |
