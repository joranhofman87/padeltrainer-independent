...
| 7b | *(owner)* prove the DEPLOYED endpoint returns exactly `{"state":"cron_disarmed"}` while the cron is still inactive | 503 alone is ambiguous — `query_failed`, `misconfigured` and `stale` share it. This is the step that proves the expectation actually propagated. |
