...
// a PGlite Postgres seeded with one row per bug pattern + clean/paid controls, and
// asserts each detector flags exactly the right invoices (and nothing else). Proves
// the query is correct + parses on real PG before it touches prod data.
//
// IMPORTANT: every seeded invoice has cycle_id = NULL, mirroring prod (the 2026-07
// run found invoices.cycle_id null on all 42 unpaid rows). The detectors run UNSCOPED
// (no cycle JOIN) precisely so a null cycle_id cannot hide a corrupt invoice — an
// earlier cycle_id-scoped version returned a false 0.
...
// Minimal schema with only the columns the reconciliation references. cycle_id is
// present (mirrors the real schema) but seeded NULL everywhere — the detectors must
// still catch corruption without it.
await db.exec(`
  ...
  -- CLEAN: 1 player, total = subtotal+vat, split_count 1. Flagged by NOTHING.
  INSERT INTO invoices (id,invoice_number,cycle_id,status,split_count,line_items,subtotal,vat_amount,total,booking_ids,player_id)
    VALUES ('a0000000-0000-0000-0000-000000000001','INV-CLEAN',NULL,'sent',1,
  ...
  INSERT INTO invoices (id,invoice_number,cycle_id,status,split_count,line_items,subtotal,vat_amount,total,booking_ids,player_id)
    VALUES ('a0000000-0000-0000-0000-000000000002','INV-A1',NULL,'sent',1,
  ...
  INSERT INTO invoices (id,invoice_number,cycle_id,status,split_count,line_items,subtotal,vat_amount,total,booking_ids,player_id)
    VALUES ('a0000000-0000-0000-0000-000000000003','INV-B2',NULL,'draft',2,
      '[{"description":"Sessie","amount":121}]'::jsonb,100,21,121,ARRAY['${bA}']::uuid[],'${ALICE}');

  -- B3: total (130) != subtotal+vat (121). Flagged by Q4 only.
  INSERT INTO invoices (id,invoice_number,cycle_id,status,split_count,line_items,subtotal,vat_amount,total,booking_ids,player_id)
    VALUES ('a0000000-0000-0000-0000-000000000004','INV-B3',NULL,'overdue',1,
      '[{"description":"Sessie","amount":130}]'::jsonb,100,21,130,ARRAY['${bA}']::uuid[],'${ALICE}');

  -- PAID control: every corruption, but status='paid' → excluded everywhere by the status filter.
  INSERT INTO invoices (id,invoice_number,cycle_id,status,split_count,line_items,subtotal,vat_amount,total,booking_ids,player_id)
    VALUES ('a0000000-0000-0000-0000-000000000005','INV-PAID',NULL,'paid',2,
...
ok(Number(r0.unpaid_invoices) === 4, 'Q0: 4 unpaid invoices (clean+A1+B2+B3; paid excluded)', r0.unpaid_invoices);
...
ok(a1rows.length === 1 && a1rows[0].invoice_number === 'INV-A1' && Number(a1rows[0].distinct_players) === 2,
  'Q2: flags ONLY INV-A1 (2 distinct players, despite null cycle_id)', a1rows.map((r) => r.invoice_number));
