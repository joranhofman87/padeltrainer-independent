-- ============================================================================
-- get_academy_cyclus_groups — add a derived `kind` (Rebook / Registration / Event / Cyclus)
-- ============================================================================
-- The cycle overview is about to flood with summer rebook rounds + a full slot generation, and
-- there's no quick way to tell kinds apart — rebook rounds are `type='cyclus'` (same as training),
-- registrations are flattened into `group_type='cyclus'`, so the existing `group_type` can't
-- separate them. This adds a first-class `kind` column the overview surfaces as a sortable +
-- filterable Type column:
--   'rebook'       — a rebook round (settings.rebook_payment_mode / rebook_round_id present)
--   'registration' — a sign-up form (is_registration)
--   'event'        — a one-off event (cycles.type = 'event')
--   'cyclus'       — everything else (training cycli + calendar-generated free-booking slots;
--                    individual-vs-group is already the overview's booking-mode column)
-- Rebook is checked FIRST because a rebook round is born type='cyclus'. Everything else in the
-- function is re-emitted verbatim from 20260816100000 (person-keyed roster + hold-aware counts).
-- ============================================================================

-- Adding `kind` to RETURNS TABLE changes the function's return type, which CREATE OR REPLACE
-- cannot do — drop first, then recreate. (No view/other object depends on this RPC.)
...
cyc AS (
  ...
  SELECT c.id, c.name, c.owner_id, c.owner_type, c.status, c.type, c.settings,
...
s AS (
  SELECT
    ...
    -- a rebook round is born type='cyclus'; flag it from its run-state settings so `kind` can
    -- separate the summer rebook flood from plain training cycli.
