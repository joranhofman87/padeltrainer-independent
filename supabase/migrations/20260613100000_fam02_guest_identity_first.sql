-- FAM-02 (owner decision 2026-06-12): a linked guest record shows the
-- PERSON's own identity, not the linked account's. Families share one email,
-- so a child's guest record can be linked to the parent's account; the
-- previous profile-first coalesce made kids appear under the parent's name
-- and rating in player lists. Person-specific fields (name, rating, rating
-- system) are now guest-first with profile fallback; account-level contact
-- fields (email, phone, billing) stay profile-first; birth_date was already
-- guest-first. Otherwise identical to 20260612142000.
...
BEGIN
  ...
  base AS (
    ...
    SELECT
      ...
      coalesce(nullif(btrim(g.full_name), ''), lp.full_name)      AS b_full_name,
      ...
      coalesce(g.skill_rating, lp.skill_rating)                   AS b_skill_rating,
      coalesce(nullif(g.rating_system, ''), lp.rating_system, 'knltb') AS b_rating_system,
      g.notes                                                     AS b_notes,
