-- Phase 0c round-2 hardening (second external audit of the twin bridge). Three fixes:
--
-- (1) RE-LOCK the raw can_book_member_window(_user_id, _cycle_id). 20260717100000 locked it to
--     service_role (arbitrary-_user_id + SECURITY DEFINER = an eligibility oracle for anon) and
--     gave clients the auth.uid() wrapper can_current_user_book_member_window. The LATER
--     20260731100000 re-created the function and mistakenly re-granted anon/authenticated —
--     re-opening the oracle IN PROD since then — and 20260826230000 copied that grant forward.
--     Restore the 20260717100000 grant state. The client calls only the wrapper
--     (src/lib/slotVisibility.ts); edge fns run as service_role; can_book_slot/the booking trigger
--     call it via SECURITY DEFINER ownership — nothing legitimate loses access.
--
-- (2) READ-TIME PRECEDENCE: a row's explicit twin_of_profile_id now OUTRANKS its email-inferred
--     linked_profile_id in ALL FOUR player-side readers. A conflicted row (linked = parent from
--     the no-name-guard email trigger, twin = child from an explicit manager claim) is the child's
--     row — showing it to the parent leaks the child's bookings/invoices/rebook eligibility.
--     Conflicted rows arise directly from claiming an already-mislinked row (and the link trigger
--     may re-infer a link on later email changes), so the readers must enforce the precedence
--     themselves — the same twin-outranks-linked rule the merge guard already applies.
--
-- (3) merge_guest_players survivor hygiene: don't carry an inferred link that CONFLICTS with the
--     effective twin onto the surviving row (readers would be protected by (2) anyway, but the
--     conflicted state should not be manufactured). Also extend the repurpose guard: clearing the
--     twin on RENAME (20260826210000) missed an email-only repurpose — changing a stamped row's
--     email AWAY from the twin profile's email now detaches the stamp too (corrections TOWARD the
--     profile's email, case-only changes, emptying the email, and emailless profiles keep it).

-- ---------------------------------------------------------------------------
-- (2a) get_my_linked_guest_bookings — twin outranks linked per row.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- (2b) get_my_paid_booking_ids — same precedence.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- (2c) get_my_pending_priority_claims — same precedence.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- (2d) can_book_member_window clauses (d)/(e) — same precedence. (1) grant state restored below.
-- ---------------------------------------------------------------------------
...
AS $$
  ...
  SELECT
    ...
    -- (d) a GUEST claim in this round whose guest is twinned — or, when unstamped, linked — to me
    OR EXISTS (
      SELECT 1
      FROM public.slot_priority_claims spc
      JOIN public.availability_slots s ON s.id = spc.slot_id
      ...
      WHERE s.source_cycle_id = _cycle_id
        AND (gp.twin_of_profile_id = (SELECT id FROM me)
             OR (gp.twin_of_profile_id IS NULL AND gp.linked_profile_id = (SELECT id FROM me)))
    )
    -- (e) an accountless GUEST on this round's priority list, twinned — or, when unstamped,
    --     linked — to me
    OR EXISTS (
      SELECT 1
      FROM public.cycles c
      JOIN public.guest_players gp
        ON (gp.twin_of_profile_id = (SELECT id FROM me)
            OR (gp.twin_of_profile_id IS NULL AND gp.linked_profile_id = (SELECT id FROM me)))
      WHERE c.id = _cycle_id
...
COMMENT ON FUNCTION public.can_book_member_window(uuid, uuid) IS
  'Second-bucket (member window) booking eligibility: existing rebooker OR original cohort (registered claim) OR registered priority list OR a twinned/linked ex-guest cohort member (guest claim in the round) OR a twinned/linked guest on the priority-guest list. A row''s explicit twin_of_profile_id outranks its inferred linked_profile_id. Pure authorization gate — never affects capacity. LOCKED to service_role (20260717100000); clients use can_current_user_book_member_window.';

-- (1) restore the 20260717100000 grant state (explicit named revoke — Supabase default
-- privileges auto-grant EXECUTE to anon/authenticated on creation).
...
-- the client-safe wrapper stays available (re-assert; unchanged since 20260717100000)
...
-- ---------------------------------------------------------------------------
-- (3a) repurpose guard: rename AND email-divergence now detach the stamp.
-- ---------------------------------------------------------------------------
...
-- replace the rename-only trigger from 20260826210000
DROP TRIGGER IF EXISTS trg_clear_guest_twin_on_rename ON public.guest_players;
DROP FUNCTION IF EXISTS public.clear_guest_twin_on_rename();
...
CREATE TRIGGER trg_clear_guest_twin_on_repurpose
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.clear_guest_twin_on_repurpose();

-- ---------------------------------------------------------------------------
-- (3b) merge_guest_players: don't manufacture a conflicted survivor. Body byte-identical to
--      20260826220000 except the linked_profile_id carry (see the CASE at the bottom).
-- ---------------------------------------------------------------------------
