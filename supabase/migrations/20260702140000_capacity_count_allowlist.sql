-- #181 (A1 adversarial-review follow-up): align every DB slot-capacity COUNT to the
-- canonical occupancy ALLOWLIST.
--
-- All five DB capacity counts used a DENYLIST — `COALESCE(status,'confirmed') NOT IN
-- ('cancelled','cancelled_swap')` — which counts `rejected` and `completed` rows toward
-- occupancy. The frontend's single source of truth, CAPACITY_OCCUPYING_STATUSES
-- (src/lib/lessons.ts), is an ALLOWLIST: ['confirmed','pending','pending_approval']
-- ("cancelled / cancelled_swap / rejected / completed do not hold a future seat"), and the
-- partial index idx_bookings_slot_status (migration 20260629120000) already uses that
-- allowlist. So a slot at denylist-capacity *because of* a stale rejected/completed row
-- blocked a staff add the UI showed as open. Fail-closed, but wrong.
--
-- Fix: flip the five counts to `COALESCE(status,'confirmed') IN
-- ('confirmed','pending','pending_approval')` so server occupancy == the app's occupancy
-- == the index predicate. This only LOOSENS capacity (counts fewer rows), so it can never
-- newly overbook. The five sites are: enforce_booking_slot_tier (the BEFORE INSERT/UPDATE
-- trigger), book_slot_for_payment (online-pay RPC), respond_to_priority_claim's two counts
-- (legacy + group accept), and swap_member_booking (the member-swap RPC; uncalled in the
-- current UI but kept consistent). Everything else in these functions is byte-identical to
-- their current definitions (20260702120000 / 20260701130000 / 20260614110000 /
-- 20260506095426); only the count predicate changes. The UPDATE short-circuit's cancelled-set
-- check is intentionally left alone — it detects the un-cancel TRANSITION, not occupancy.
--
-- status is NOT NULL on bookings, so the COALESCE is a no-op kept for parity; the predicate
-- is value-equivalent to the index's `status IN (...)`.
...
BEGIN
  ...
  WHERE slot_id = NEW.slot_id
    AND id <> NEW.id
    AND COALESCE(status, 'confirmed') IN ('confirmed', 'pending', 'pending_approval');
...
-- Byte-identical to 20260506095426 except the count predicate. CREATE OR REPLACE preserves
-- existing grants. (Uncalled in the current UI, aligned for consistency + future revival.)
...
BEGIN
  ...
  -- Capacity check
  IF (SELECT COUNT(*) FROM bookings WHERE slot_id = _new_slot_id AND COALESCE(status,'confirmed') IN ('confirmed','pending','pending_approval')) >= COALESCE(v_new_slot.max_participants, 1) THEN
