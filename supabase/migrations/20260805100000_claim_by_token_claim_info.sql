-- ============================================================================
-- REBOOK · return rebook_claim_info (custom claim-page text) from get_priority_claim_by_token
-- ============================================================================
-- WHY: the claim page's "Hoe werkt het? / Wil je een ander moment?" box is hard-coded copy.
-- Academies want to write their own explanation per round (settings.rebook_claim_info, set by
-- the wizard or the round texts editor). The claim page is anon + status-independent, so the
-- text must ride the token RPC like rebook_rules does.
--
-- This re-emits get_priority_claim_by_token VERBATIM from 20260723100000 and only ADDS the
-- 'rebook_claim_info' key (plain text; NULL/empty ⇒ frontend shows the standard copy). The
-- frontend falls back to the default text when the key is absent, so it is safe to ship the
-- frontend before this migration.
-- ============================================================================
...
BEGIN
  SELECT jsonb_build_object(
    ...
    -- Academy-authored claim-page explanation (plain text). NULL/empty ⇒ standard copy.
