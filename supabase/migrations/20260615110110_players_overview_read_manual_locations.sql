-- Player locations Phase 2: the displayed location array also reads the academy-curated
-- store academy_player_locations — manually ATTACHED clubs (dismissed=false) join the
-- union; manually-dismissed clubs (dismissed=true) are subtracted even when trained/
-- preferred/intake would surface them. Filter mirrors it. Body = 20260615110090 + those.
-- (original Phase-1 note kept below.)
-- Player locations = single source of truth (Phase 1). The location array shown in the
-- players table now unions, per player: TRAINED (confirmed/completed booking -> slot
-- location, kept gated to ACTIVE academy locations as before) + PREFERRED club
-- (guest_players / academy_player_metadata.preferred_location_id) + ENROLLED-intake
-- (intake_requests.location_id). Deliberately-set preferred/intake locations show even
-- when the academy location is inactive (must still belong to the academy); merged
-- locations resolve to canonical. The location FILTER is extended to match the same set
-- (so filtering by a club returns exactly the players whose chip shows it). RETURNS shape
-- is unchanged -> plain CREATE OR REPLACE; body is 20260615110020 verbatim + these edits.
...
COMMENT ON FUNCTION public.get_players_overview(text, uuid, text, jsonb, text, text, integer, integer) IS
  'Players overview (academy/trainer scope): membership per the former client core (guests + registered, removal-filtered, linked-deduped), canonical linked-profile fields, server-side search/filters/sort/pagination. Location array unions trained + preferred + intake + manual-attach store, minus dismissed (Phase 2). SECURITY DEFINER with explicit scope authorization.';
