-- Merge two guest players into one: every relation (bookings, invoices,
-- intake requests, priority claims, academy/trainer metadata) moves to the
-- target, chosen personal fields are applied, and the source is deleted.
--
-- Authorization mirrors get_players_overview: academy managers may merge any
-- two players in their academy's membership (academy-level or active-trainer
-- guests), trainers may merge their own guests.
--
-- p_fields: personal values the admin chose to keep, applied to the target.
-- Allowed keys: full_name, first_name, last_name, email, phone, skill_rating,
-- rating_system, birth_date, notes, billing_business_name, billing_address,
-- billing_btw_number, preferred_location_id, source. Omitted keys keep the
-- target's current value. has_trained is OR-combined; linked_profile_id is
-- coalesced (merging two guests linked to DIFFERENT accounts is rejected).
--
-- Email-campaign history is keyed by email address, so it follows whichever
-- email is kept. Runs atomically; the guest-link trigger fires on the target
-- update and performs its normal idempotent linking.
...
BEGIN
  ...
  ------------------------------------------------------------------
  -- Resolve email before deleting the source (partial unique indexes):
  -- clearing the source's email first makes any kept email conflict-free.
  ------------------------------------------------------------------
  v_keep_email := CASE
...
COMMENT ON FUNCTION public.merge_guest_players(text, uuid, uuid, uuid, jsonb) IS
  'Merge duplicate guest players: repoints bookings/invoices/intakes/claims, merges metadata (tags union, notes appended), applies chosen personal fields, deletes the source. SECURITY DEFINER with explicit scope authorization.';

REVOKE ALL ON FUNCTION public.merge_guest_players(text, uuid, uuid, uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.merge_guest_players(text, uuid, uuid, uuid, jsonb) TO authenticated;
