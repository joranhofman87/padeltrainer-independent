...
--
-- The create-guest-* edge functions already pre-check, so a guest gets a clean refusal before
-- anything is created. But a pre-check is not a boundary: these RPCs are the only thing between
-- any future caller and a booking row, they take no user id so they never reach can_book_slot,
-- and edge-only enforcement left the guest path with ONE layer where the registered path has
-- three (trigger, book_slot_for_payment, edge pre-check). Re-emitted here with the guard first.
--
-- Bodies are otherwise byte-for-byte their latest deployed definitions — generated by patching
-- that text rather than retyping it — and the signatures are unchanged, so no types drift.
-- Grants are unchanged too (service_role only), so they are not re-stated.
...
BEGIN
  ...
  -- BOOKING CUTOFF at the mutation boundary. The create-guest-* edge functions check first so
  -- the guest gets a clean message before anything is created, but this is the HARD boundary:
  -- these RPCs take no user id and so never reach can_book_slot, which carries the rule for
  -- registered players. Without this, any future caller of this function bypasses the cutoff
  -- entirely — the registered path has three layers, and the guest path had one.
  IF public.is_slot_within_player_booking_cutoff(_slot_id) THEN
...
BEGIN
  ...
  -- BOOKING CUTOFF at the mutation boundary (see book_guest_slot_for_payment). ANY slot inside
  -- its cutoff refuses the whole purchase: selling someone a cycle and silently dropping the
  -- session that starts tomorrow would be worse than refusing the sale.
  IF EXISTS (
    SELECT 1 FROM unnest(_slot_ids) AS sid
    WHERE public.is_slot_within_player_booking_cutoff(sid)
  ) THEN
    RAISE EXCEPTION 'booking_cutoff' USING ERRCODE = 'check_violation';
  END IF;
