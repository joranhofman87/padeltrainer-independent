-- Email delivery tracking — surface "email is undeliverable" on the players overview.
-- Adds email_undeliverable (parallel to has_overdue_payment): TRUE when the row's
-- resolved email has a hard_bounced/complained address-state. Changing the RETURNS
-- TABLE shape requires DROP + recreate; the body below is the original function
-- (20260611160001) verbatim with the one new column + one new LATERAL.
