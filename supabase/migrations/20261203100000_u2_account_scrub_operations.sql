...
--     was immutable and the row was append-only, so the operation could not finish until database
--     time passed the caller's stamp, and the one-active-operation index blocked any replacement
--     meanwhile. An hour of clock skew meant an hour in which that account could not be erased.
...
CREATE TABLE public.account_scrub_operations (
  -- A single uuid `id`, because scripts/db/backup-coverage.mjs refuses a backed-up table shaped
  -- any other way. (The export orders by id; it does not page on it.)
  ...
  CONSTRAINT account_scrub_operations_state_shape_check CHECK (
    ...
    OR (
      -- A claim is outstanding, and lease_expires_at says until when.
...
BEGIN
  ...
  -- Everything below requires an unexpired lease. Note what that does and does not buy: a
  -- transition made after expiry is refused, but nothing here checks WHICH worker is making it —
  -- that predicate belongs to the RPCs. See the lease_expires_at comment.
  IF OLD.lease_expires_at <= v_wall_now THEN
