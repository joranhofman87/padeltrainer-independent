-- ============================================================================
-- get_academy_cyclus_groups — scope by academy, not active-trainer (audit Batch 5, §4.5)
-- ============================================================================
-- The overview RPC (SECURITY DEFINER; caller IDOR-checked as a manager of p_academy_id) scoped its
-- slots + cycles by the academy's ACTIVE trainer membership. Two bugs:
--   • CROSS-TENANT LEAK: a trainer active in academy A *and* B dragged B's slots + bookings + intake
--     PLAYER NAMES into A's overview (the slot filter was `trainer_id IN active_trainers`, never the
--     slot's own academy).
--   • Departed trainers' cycles reported "no players" — their slots were filtered out the moment they
--     left, even though the slots still belong to the academy.
--
-- Fix: the tenant boundary is the SLOT's `academy_profile_id`. Scope slots to
-- `academy_profile_id = p_academy_id`; scope cycles to academy-owned OR referenced by one of those
-- slots; resolve trainer names from the slots' own trainer_ids (active or departed). A shared
-- trainer's OTHER academy's slots (academy_profile_id = B) are now excluded, and a departed trainer's
-- academy slots stay. Everything else is re-emitted verbatim.
-- ============================================================================
