-- ═══════════════════════════════════════════════════════════════════════════════════════════
-- FINAL INTEGRATION AUDIT — the N2 <-> N3 seam: the resolver must honour the one-click
-- unsubscribe that N2's own footer promises.
--
-- N2 gave marketing mail an unsubscribe footer and an RFC 8058 one-click header, and records the
-- result in email_marketing_suppression. N3's resolver honoured the DELIVERABILITY suppression
-- (hard bounce / complaint, is_email_suppressed) but never read the marketing one. No producer
-- enqueues a marketing-category event through this path today — the closed producer inventory in
-- notificationAttributionMatrix.test.ts has none — so nothing was mis-sent. But `marketing_updates`
-- sits in the catalogue with supports_email = true and email_footer_policy =
-- 'marketing_unsubscribe', so the first producer to use it would have shipped a promise the
-- pipeline does not keep. A contradiction between two units is exactly what a final integration
-- audit is for, and it is closed here rather than recorded.
--
-- Recreated whole from 20261015120000 (its newest definition); only the email branch changes.
-- ═══════════════════════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.enqueue_notification(text, uuid, uuid, uuid, uuid, uuid, text, uuid[], uuid, text, text, jsonb, jsonb, timestamptz) IS
  'The resolver: one event in, per-channel decisions out. Skips are terminal and REASONED — preference_off, tenant_restricted, no_email_contact, email_suppressed (bounce/complaint), marketing_unsubscribed (the one-click unsubscribe N2 records, honoured for events whose footer policy promises it), no_deliverable_channel.';
