-- 10c-a3 PR-1 (SQL reliability) — canonical suppression readers.
--
-- WHY: 20261006100000 made email_address_state.is_suppressed the SINGLE source of truth
--   (= state IN ('hard_bounced','complained') OR provider_suppressed_active). Two per-ADDRESS delivery-issue readers
--   still derived the flag from the RAW `state` column and therefore MISS a Resend provider-list suppression
--   (email.suppressed with no bounce). Re-emit both to reference the canonical generated column so every operator
--   surface agrees. Bodies are byte-identical to their latest definitions except the single predicate swap.
--   (get_invoice_delivery_status is intentionally NOT re-emitted: it is a per-INVOICE latest-EVENT status from
--   email_delivery_events, a different semantic that is_suppressed cannot express — see the PR-1 report.)
-- CREATE OR REPLACE preserves privileges; the REVOKE/GRANT below re-assert the intended ACL explicitly.
...
BEGIN
  ...
  SELECT DISTINCT ON (mem.m_player_key)
         ...
         mem.m_full_name, mem.m_email,
         -- TRUTHFUL status: a bounce/complaint reports its state; a provider-only suppression (state='ok' but on
         -- Resend's suppression list) reports 'provider_suppressed' — never a bare 'ok' the client miscasts.
         CASE WHEN s.state IN ('hard_bounced', 'complained') THEN s.state ELSE 'provider_suppressed' END AS state,
         s.last_event_at
  FROM members mem
