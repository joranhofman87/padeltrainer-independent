...
COMMENT ON FUNCTION public.get_players_overview(text, uuid, text, jsonb, text, text, integer, integer) IS
  'Players overview (academy/trainer scope): membership per the former client core (guests + registered, removal-filtered, linked-deduped), canonical linked-profile fields, server-side search/filters/sort/pagination. Location array unions trained + preferred + intake (Phase 1). SECURITY DEFINER with explicit scope authorization.';
