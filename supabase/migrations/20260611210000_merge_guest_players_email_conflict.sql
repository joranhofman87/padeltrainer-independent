-- merge_guest_players v2: detect kept-email collisions with a THIRD player.
--
-- Production case: keeping the source's email failed with a raw unique-index
-- violation when another guest under the target's owner scope already held
-- that email (duplicates can legitimately exist once per scope — e.g. one
-- record under the trainer and one under the academy). Now the merge
-- pre-checks and raises a clear error naming the conflicting player, so the
-- admin knows to merge or fix that record first. ERRCODE 23505 kept so
...
BEGIN
  ...
  ------------------------------------------------------------------
  -- Kept-email pre-check: a THIRD guest under the target's owner scope may
  -- already hold the email being kept (duplicates can exist once per scope).
  -- Fail early with the conflicting player's name instead of a raw
  ...
  IF v_keep_email IS NOT NULL THEN
    SELECT gp.full_name INTO v_conflict_name
    FROM public.guest_players gp
    WHERE gp.id NOT IN (p_source_guest_id, p_target_guest_id)
      AND lower(btrim(coalesce(gp.email, ''))) = lower(btrim(v_keep_email))
      AND ((v_target.trainer_id IS NOT NULL AND gp.trainer_id = v_target.trainer_id)
        OR (v_target.trainer_id IS NULL AND v_target.academy_profile_id IS NOT NULL
            AND gp.academy_profile_id = v_target.academy_profile_id AND gp.trainer_id IS NULL))
    LIMIT 1;
    IF v_conflict_name IS NOT NULL THEN
      RAISE EXCEPTION 'email is already used by another player: %', v_conflict_name
        USING ERRCODE = '23505';
    END IF;
  END IF;
