-- Phase 5: a NON-DESTRUCTIVE payment reconciliation report. reconcile_payments() runs a set of
-- read-only checks over local DB state and RETURNS the mismatches it finds — it NEVER writes or fixes
-- anything. Admin-only (SECURITY DEFINER + has_role gate). Intended for a daily operator check /
-- dashboard. See docs/payments/PAYMENT_RECONCILIATION_PLAN.md (incl. the live-Mollie limitation:
-- "Mollie says paid but our invoice is unpaid" needs the Mollie API and is out of scope here).
--
-- Returns one row per finding: (check_name, severity, entity_kind, entity_id, detail).
