-- Notification Foundation v2 — PR 3: the policy resolver (enqueue_notification).
-- See docs/NOTIFICATION_ARCHITECTURE.md §3 (reconciliation) + §6 (PR sequence).
--
-- This is the SINGLE place feature/payment code turns "intent" into concrete
-- notification_outbox rows. It owns every policy decision so callers stay dumb:
--   * recipient normalization across the person-unification dual keys
--     (person_id / user_id / guest_player_id → the one person),
--   * per-channel preference resolution (prefs_v2 override else event default),
--   * required-delivery guarantee (the email channel can't be turned off/digested),
--   * consent: transactional email needs no opt-in (only respects hard suppression),
--     whatsapp/push REQUIRE an opted-in contact whose consent is IN TENANT SCOPE
--     (is_notification_consent_in_scope — persons are global, so a raw opt-in leaks),
--   * PER-RECIPIENT idempotency: key = <event>:<subject>:<recipient>, channel is the
--     UNIQUE constraint (not the key) → re-enqueue is a no-op, one key per recipient,
--   * a visible 'skipped' row when a REQUIRED event has no deliverable channel.
--
-- Deferred to later PRs (documented so reviewers don't read them as gaps here):
--   * digest BATCHING + quiet-hours + max_per_user rate limits → the worker (PR 4+);
--     this resolver only sets scheduled_for/collapse_key so the worker can honor them.
--   * the LEGACY send-email `type` → v2 `key` map → ships with the first pilot (PR 5),
...
CREATE OR REPLACE FUNCTION public.notification_redact_destination(p_value text, p_channel text)
RETURNS text
LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_value IS NULL OR btrim(p_value) = '' THEN NULL
    WHEN p_channel = 'email' AND position('@' IN p_value) > 1
      THEN left(split_part(p_value, '@', 1), 1) || '***@' || split_part(p_value, '@', 2)
    WHEN p_channel IN ('whatsapp', 'push') THEN
      CASE WHEN length(regexp_replace(p_value, '\D', '', 'g')) >= 4
        THEN '•••' || right(regexp_replace(p_value, '\D', '', 'g'), 4)
        ELSE '•••' END
    ELSE '***'
  END;
$$;
COMMENT ON FUNCTION public.notification_redact_destination(text, text) IS
  'Notification v2: tenant-safe redaction of a raw destination (email → j***@x.com, phone → •••1234). Used for the persons.email fallback where no contact row supplies destination_redacted. service_role only.';
REVOKE ALL ON FUNCTION public.notification_redact_destination(text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.notification_redact_destination(text, text) TO service_role;

-- ---------------------------------------------------------------------------
-- enqueue_notification — intent → idempotent, consent-checked outbox rows.
