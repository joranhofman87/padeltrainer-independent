-- P1-3: merge_guest_players destroyed CASCADE children.
--
-- The previous body (20260612140000) repointed bookings/invoices/intake_requests/
-- slot_priority_claims/academy_player_metadata then DELETEd the source guest, but two
-- ON DELETE CASCADE children added ~2026-06-15 were never repointed and were therefore
-- destroyed on every merge:
--   * session_player_notes.subject_guest_player_id  (20260615100000, CASCADE)
--   * academy_player_locations.guest_player_id       (20260615110100, CASCADE)
-- This CREATE OR REPLACE reproduces the exact prior body and adds their repoint BEFORE
-- the source-email-clear + DELETE. It also repoints the two ON DELETE SET NULL group-
-- captain attribution markers so the surviving guest inherits "who booked/paid for whom"
-- instead of having it nulled:
--   * slot_priority_claims.booked_by_guest_player_id (20260626100000, SET NULL)
--   * bookings.paid_by_guest_player_id               (20260626110000, SET NULL)
-- Signature, SECURITY DEFINER, search_path and the manager/trainer auth gate are unchanged.
