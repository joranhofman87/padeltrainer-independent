-- U2 — the create-command record survives an operator merge.
--
-- `merge_guest_players` is the reviewed, operator-invoked merge (20261115100000). It repoints every
-- guest-keyed row onto the target and then deletes the source guest, which lets the last-source
-- cleanup trigger destroy the source person. `player_create_commands` references both, so both of
-- its answers were being nulled by the FKs — and a nulled `person_id` is how the command tells a
-- retry that its Player is GONE. After a merge the Player is not gone; it is the target.
--
-- Reproduced from 20261115100000 mechanically. `src/test/mergeGuestPlayersReproduction.test.ts`
-- re-derives this body from that file and fails if the two diverge by anything other than the
-- inserted block. Retyping a 340-line function from memory is how a column silently disappears —
-- and a reviewer cannot see an omission in a wall of unchanged SQL, so a machine checks it.
...
BEGIN
  ...
  -- Player that is alive under the target's id. Repointed here, where both successors are known
  -- exactly, and only when the source person actually dies: a source person that survives the merge
  -- keeps its own commands.
