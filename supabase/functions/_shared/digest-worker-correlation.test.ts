...
import { runDigestWorker, type WorkerDeps, type WorkerSummary } from "./digest-worker-core.ts";
...
function deps(recordReturns: unknown, opts: { httpStatus?: number } = {}) {
  ...
  const d: WorkerDeps = {
    ...
    } as Awaited<ReturnType<WorkerDeps["sendOnce"]>>),
    now: () => new Date("2026-08-03T12:00:00Z"),
    monotonicNowMs: () => 0,
    newToken: () => "token-1",
    log: (e) => logs.push(e),
  };
  return { d, logs, rpcCalls };
}

const finishedWith = (rpcCalls: string[]) => rpcCalls.filter((n) => n === "finish_notification_worker_run").length;

Deno.test("a correlation mismatch makes the RUN unhealthy (not a green 200)", async () => {
  const { d, rpcCalls } = deps("correlation_mismatch");
  ...
  assert(finishedWith(rpcCalls) >= 1);
});
