...
Deno.test("2xx accepted → response with provider message id, no error", async () => {
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: fakeFetch({ status: 202, body: { id: "re_abc" } }) });
...
Deno.test("2xx WITHOUT a usable email id → 'no_response' transport (never a false accepted)", async () => {
  ...
  for (const body of [{}, { id: "" }, { id: "   " }, { id: 12345 }, { id: null }] as unknown[]) {
    const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: fakeFetch({ status: 202, body }) });
  ...
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: badJson });
  assertEquals(r.kind, "transport");
  if (r.kind === "transport") assertEquals(r.transport, "no_response");
});
...
Deno.test("exactly ONE HTTP call is made (no internal retries)", async () => {
  ...
  await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: counting });
  assertEquals(calls, 1);
});

Deno.test("422 validation → error_name from body.name, no provider id", async () => {
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: fakeFetch({ status: 422, body: { name: "validation_error", message: "bad" } }) });
...
Deno.test("429 rate limit → error_name + Retry-After seconds surfaced", async () => {
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: fakeFetch({ status: 429, body: { name: "rate_limit_exceeded" }, headers: { "Retry-After": "30" } }) });
...
Deno.test("error body without name falls back to http_<status>", async () => {
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: fakeFetch({ status: 500, body: {} }) });
  assertEquals(r.kind, "response");
  if (r.kind === "response") assertEquals(r.errorName, "http_500");
});

Deno.test("error_name is an ALLOW-LIST: an unknown name (or PII-bearing message) never leaks → http_<status>", async () => {
  // an unrecognised `name` is NOT echoed (it could carry data) — it collapses to http_<status>...
  const unknown = await sendResendEmailOnce("k", PAYLOAD, {
    idempotencyKey: KEY, groupId: GID,
    fetchImpl: fakeFetch({ status: 400, body: { name: "some_unexpected_code", message: "delivery to alice@victim.com failed" } }),
  });
  assertEquals(unknown.kind, "response");
  if (unknown.kind === "response") assertEquals(unknown.errorName, "http_400");
  // ...and the free-text `message` (which can contain a recipient email) is NEVER used, even with no `name`.
  const msgOnly = await sendResendEmailOnce("k", PAYLOAD, {
    idempotencyKey: KEY, groupId: GID,
    fetchImpl: fakeFetch({ status: 400, body: { message: "bob@example.com bounced" } }),
  });
  if (msgOnly.kind === "response") {
    assertEquals(msgOnly.errorName, "http_400");
    if (msgOnly.errorName) assertEquals(msgOnly.errorName.includes("@"), false);
  }
});

Deno.test("EVERY request carries the stable digest_group_id tag = the group id", async () => {
  ...
  await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: capture });
  assertEquals(sentBody!.tags, [{ name: "digest_group_id", value: GID }]);
  // the frozen request fields ride through unchanged alongside the tag
  assertEquals(sentBody!.from, PAYLOAD.from);
  assertEquals(sentBody!.to, PAYLOAD.to);
});
...
Deno.test("timeout (AbortError) → transport 'timeout'", async () => {
  ...
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, timeoutMs: 5, fetchImpl: hangingFetch });
  assertEquals(r.kind, "transport");
...
Deno.test("network error → transport 'network'", async () => {
  ...
  const r = await sendResendEmailOnce("k", PAYLOAD, { idempotencyKey: KEY, groupId: GID, fetchImpl: boom });
  assertEquals(r.kind, "transport");
