...
serve(async (req) => {
  ...
  try {
    ...
    const composedState = `trainer_${trainerProfile.id}_${state}`;
    ...
    // Alert on any connect-token / OAuth-state failure (best-effort, never throws)
    await notifySlackEdgeError("mollie-connect-trainer", errorMessage);
